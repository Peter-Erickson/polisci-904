...
#Now Prep the data that I need to analyze polarization levels.  I will try first by 
...
#next, conduct a logit regression where the DV is values of Ques_Prof along
#several IVs

logmodel <- glm(
  Ques_Prof ~ Inst_Cred_Gallup + House_distance_dim1 + MilPSoc + Dom_P + Fig_Nme, 
...
# now  make prediction data using the mean institutional credibility scores.  Vary #polarization and the three topics I am interested in.  
  
predictiondata <- tibble(House_polarization_level=seq(min(slimdata$House_distance_dim1), max(slimdata$House_distance_dim1), by=.01), Credibility = mean(slimdata$Inst_Cred_Gallup)) %>%
crossing(Ques_Prof = c(0,1), MilPSoc = c(0,1), Dom_P = c(0,1), Fig_Nme = c(0,1))
...
augment(logmodel, newdata = predictiondata)

predictions <- augment(logmodel, newdata=predictiondata) %>% 
  mutate(
    .prob=plogis(.fitted),
...
predictions %>% arrange(.prob, .fitted, Report, Night, desc(.fitted)) %>% 
  view()
