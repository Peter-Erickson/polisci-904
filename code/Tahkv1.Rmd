...
#Determinants of Military Role Beliefs

Theoretically, there are at least two major determinants of military role beliefs, or the conceptions held by members of the military regarding their normative role in politics [@fitch_armed_1998]. In the most general terms, the military's involvement in politics requires both the _motive_ and the _opportunity_ to do so [@finer_man_1962]. I argue that rising political polarization serves as the _motive_ for the military's intervention in politics, while high levels of prestige, or credibility of the military institution, provide the _opportunity_ for doing so. It is important to elaborate why each of these offers the military in particular the ability to intervene in politics.^[For a richer elaboration on the mechanisms afforded by political polarization and institutional levels of prestige for intervention in politics, please see the author's theory chapter in his dissertation. The proceeding paragraphs here are intended to summarize these mechanisms.]  

With respect to the first determinant, political polarization, the literature in American politics enables the development of two plausible relationships and corresponding mechanisms that connect polarization to  military role beliefs. These relationships and mechanisms further stem from a particular conception or definition of what polarization is and how it workd. One possible conception of polarization is that it marks the ideological widening between political elites, to include members of Congress or the leaders of political parties [@hare_polarization_2014]. According to this view of polarization, one possible mechanism by which the military intervenes in politics is by adopting a belief that elites from both dominant political parties have failed to advance and implement an effective governing agenda, and thus, the military steps in to do what political elites are not doing. This particular dynamic has been noted, in fact, by scholars in the comparative politics subfield describing the actions of militaries in other parts of the world [@fitch_armed_1998]. 
...
Given the expectations derived from theories regarding military role beliefs, the overall purpose of opinion-editorial publications, and retired military officers as members of the military community, five distinct hypotheses are derived. These include:

\singlespace
**H1: When political polarization is high, the military (or certain military figures) will increasingly criticize civilian leadership (indicating a drop in the principle of civilian control).**

\doublespace

\singlespace
**H2: When political polarization is low, the military (or certain military figures) will publicly weigh in on traditional national security debates (and not on domestic political concerns or issues outside of relevant military expertise).**

\doublespace

\singlespace
**H3: During periods of high prestige and credibility, the frequency of military involvement in quasi-political activities (such as writing op-eds) will increase relative to periods of low prestige and credibility.**

\doublespace

\singlespace
**H4: When both political polarization and high prestige are present, the military will be increasingly vocal about certain domestic ideological or partisan causes than when these factors are not present.**

\doublespace

\singlespace
**H5: During periods of active ground combat, military elites will likely comment on the war, but with less frequency on other domestic issues.**
...
The methodological approach used in this analysis consists of two broad steps. The first is to analyze the topics which retired military officers have been writing about, and the second is to determine, regardless of the topic addressed in a publication, whether an op-ed has violated the norms of non-partisanship or civilian control of the military. The critical question is whether there are patterns in the data that can be detected, and whether these patterns can be explained.  
...
```{r table2, fig.align="center", include=TRUE, echo=FALSE, warning=FALSE}
countbytype %>%
...
ggplot(data=adjdataset, mapping=aes(x=PubYr, y=dom_pol_prop)) + geom_point() + geom_smooth() + labs(title="Figure 3: Proportion of Annual Publications authored by Retired\n Military Officers that Address Domestic Political Matters, 1979-2020", x="Year", y="Proportion", color=NULL)+ geom_vline(xintercept = 1981, size=.2, colour="tomato") + 
  geom_vline(xintercept=1989, size=.2, colour="tomato") + 
  geom_vline(xintercept=1993, size=.2, colour="dodgerblue") + geom_vline(xintercept=2001, size=.2, colour="tomato") + 
  geom_vline(xintercept=2009, size=.2, colour="dodgerblue") +
  geom_vline(xintercept=2017, size=.2, colour="tomato") +
...
ggplot(polar_and_cred, aes(x=PubYr, y=value)) + geom_line(aes(color=variable)) + facet_grid(variable~., scales="free_y") + theme(legend.position = "none") +
  labs(
    title = "Figure 5: Polarization Levels in the US House of Representatives,\n Affective Polarization, and American Confidence in the Military, 1979-2020"
  )
```


Because there are only three observations written exclusively by retired female officers, and because there is no compelling logic for why the service of the author would increase the likelihood of violating norms when publishing an opinion piece, I do not control for the sex or service affiliation of the author. Controls are included, however, for the author's rank and the source in which a publication appears. 

The dependent variable is an indicator of whether the opinion piece either called for legislative action, blatantly criticized a political leader, or clearly took a partisan stance on an issue. As some may argue that calling for legislative action in itself does not violate norms of civilian control of the military or the non-partisanship of the military institution, the dependent variable in an alternate model is an indicator for whether publication criticized a political leader or took a clearly partisan stance. The estimated model therefore takes the form:


$$y = \beta_1 credibility + \beta_2polarization + \beta3source +\epsilon $$
The results of a logistic regression of this model are displayed in Table 3, below.  
...
mod1 <- glm(leg_crit_part ~ Inst_Cred_Gallup + House_distance_dim1 + Source + AuthRank + Topic, 
...
# model 2
mod2 <- glm(crit_part ~ Inst_Cred_Gallup + House_distance_dim1 + Source + AuthRank + Topic, 
    family=binomial(link="logit"),
    data=polaradjdataset
    )
...
mod3 <- glm(leg_crit_part ~ Inst_Cred_Gallup + House_distance_dim1 + AuthRank,
    family=binomial(link="logit"),
    data=polaradjdataset
    )
...
mod4 <- glm(crit_part ~ Inst_Cred_Gallup + House_distance_dim1 + AuthRank,
    family=binomial(link="logit"),
    data=polaradjdataset
    )
...
mod5 <- glm(leg_crit_part ~ Inst_Cred_Gallup + aff_polar_alt + AuthRank,
    family=binomial(link="logit"),
    data=polaradjdataset
    )
...
mod6 <- glm(crit_part ~ Inst_Cred_Gallup + aff_polar_alt + AuthRank,
    family=binomial(link="logit"),
    data=polaradjdataset
    )
...
```
\singlespace
```{r regression table, include=TRUE, echo=FALSE, results="asis", warning=FALSE}
stargazer(mod3, mod4, mod5, mod6,
          title="Table 3: Logistic Regression Results", 
          column.labels = c('Model 3','Model 4','Model 5', 'Model 6'),
          covariate.labels = c('Military Credibility', 'Congressional Polarization', 'Affective Polarization', 'Rank'),
          align=TRUE)
...
The results shown in Table 3 indicate that both measurements of polarization - Congressional and affective - are statistically significant at the .01 level. The substantive interpretation of the coefficients for polarization, however, are a bit tricky. A coefficient of .083 for Congressional Polarization in Model indicates that a 1-unit increase in the average distance between political parties is associated with a 8.7 percent increase in the likelihood of a publication authored by a retired military officer calling for legislative action, blatantly criticizing a civilian official, or expressing a clearly partisan stance. As Figure 5 shows, though, the differences in polarization levels vary with each Congress. For instance, within the House of Representatives in the 112th Congress, which began in January 2011, polarization levels were nearly six points higher than the level that existed in the 111th Congress. The estimated effects of such a drastic change in the polarization level in Congress would be associated with more than a 50 percent increase in the predicted likelihood of a publication retired by a military officer calling for legislative action, criticizing a civilian leader, or taking a clear partisan stance. 

Similary, the coefficient in Model 6 for affective polarization of .033 indicates that a 1-unit increase in the level of affective polarization is associated with a 3.4 percent increase in the likelihood of a publication authored by a retired military officer blatantly criticizing a civilian leader or taking a clear partisan stance on a given issue. And as Figure 5 also shows, changes in the level of affective polarization over the past four decades has varied. However, in between elections held every four years, it is not uncommon to see changes of 5-7 points, which would indicate a corresponding 17 - 23.8 percent increase in the likelihood of a publication authored by a retired military officer criticizing a civilian official or taking a clear partisan stance. 

Interestingly, the coefficient on military credibility is statistically significant for Model 1 only. Across other model specifications, the coefficient remains negative, indicating than an increase in the credibility of the military would be associated with a reduction in the likelihood of a publication authored by a retired military officer violating norms of civilian control and non-partisanship. But because the coefficient is not statistically signficant in Models 4-6, we cannot draw conclusions that such a relationship exists in the data.

Finally, the coefficient for rank of the author is statistically significant in Models 3 and 5, when the dependent variable includes calling for legislative action. In Models 2 and 4, when the dependent variable is whether a publication criticizes a civilian official or takes an expressly partisan stance, the coefficient is not statistically significant. This suggests that the act of calling upon Congress or the President to take specific action is clearly more associated with the rank of an author. This makes sense, as those authors who possess a greater rank likely have had experiences that have worked alongside or dealt with legislative policymakers, and because these authors feel more comfortable in calling for legislative action than those of a lower rank who probably lack such professional experience. 

## Describe the interpretation of the author, the press being willing to publish, etc. Pull from the paras below...

## Describe whether you found support for your H's or not

## Mixed Models and Source Random / Fixed Effects

a  polarization change However, is statistically significant across a graph clearly shows a non-linear relationship between increasing political polarization and the likelihood that an opinion piece authored by a retired military officer will potentially violate aspects of military professionalism. While the confidence intervals at each point estimate are relatively large, the graph demonstrates that the predicted probability of violating military professionalism is different between relative extremes along the x axis. In other words, the political polarization levels that exist today, as measured by the ideological distance between members of the two prominent political parties in the house, is predicted to have resoundingly different impacts on military professionalism than the polarization levels that existed several decades ago. The polarization levels that existed several decades ago, in the late 1970s and 1980s, may have had negligible, if any impact, on the likelihood of a retired military officer violating military professionalism in the content of a published opinion piece, but the political landscape is simply different today. Contemporary polarization levels, insofar as the predictive power of the model is concerned, changed the environment: at contemporary polarization levels (the far right of the x axis), there is roughly a 17% chance of a retired military officer's published opinion piece violating the standards and norms of military professionalism, with confidence intervals that range from roughly 11% to nearly 25%.  
